...
def align_from_xpos(run,xpos, dizi_setup, plotter):
    # Extracting positions from the input data
    ...
    # Preparing data for alignment calculation
    c = [x1, x2, y1, y2]  
    div = []  # To store the divergences
    offset_lst = []  # To store offsets for both axes
    lst_ax = ["x", "y"]
    ...
    # Loop to calculate alignment for x and y
    for i in range(2):
        # Calculating angle in micro radians
        theta = np.arctan((c[2*i+1] - c[2*i]) / d) * 1e6
        h_theta, b_thetas = np.histogram(theta, bins=1000)
        b_theta = b_thetas[:-1] + (b_thetas[1] - b_thetas[0]) / 2

        # Fitting a Gaussian to the angular data
        sigma = np.std(theta)
        mu = np.mean(theta)
        a = np.max(h_theta)
        p0 = [a, mu, sigma]
        ...
        try:
            ...
            oP = [0, 0, 0]  # Fallback values if fitting fails

        # Calculating offset to align the telescopes
        mu = oP[1]
        offset = d * np.tan(mu / 1e6)  # Converting μrad back to radians for offset calculation
        offset_lst.append(offset)
        div.append(oP[2])

        # Plotting if requested
        if plotter:
            ax[i].set_title(rf"$\theta_{{{lst_ax[i]}}}$ - run {run}")
            ...
            # Plot after applying the offset
            theta_aligned = np.arctan(((c[2*i+1] - offset) - c[2*i]) / d) * 1e6
            h_theta_aligned, b_theta_aligned = np.histogram(theta_aligned, bins=1000)
            ...
            edges = [min(mu,np.mean(theta_aligned)), max(mu,np.mean(theta_aligned))]
            ax[i].set_xlim(edges[0] - 3 * sigma, edges[1] + 3 * sigma)  # Adjust limits based on the fit
            ax[i].axvline(x=0, ls='--', c='r', alpha=0.6)
            ax[i].grid()
            ax[i].legend()
    ...
    # Ensure offset_lst and div contain two values each
    if len(offset_lst) < 2 or len(div) < 2:
        raise ValueError("Alignment calculation failed to produce valid offsets and divergences for both axes.")
...
def align(x1, y1, x2, y2, d, plotter):
    ...
    for i in range(2):
        ...
        if plotter:
            ...
            ax[i].set_xlabel(rf"$\theta_{{{lst_ax[i]}}}$ [$\mu$rad]")
            ax[i].set_ylabel('Entries')
            ax[i].set_xlim(np.mean(theta_aligned) - 3 * sigma, np.mean(theta_aligned) + 3 * sigma)  # Adjust limits based on the fit
            ax[i].grid()
            ax[i].legend()
