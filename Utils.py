        ...
        with h5py.File(data_path, 'r', libver='latest', swmr=True) as hf:
            ...
            if Year == 2022:
                if Material == 'Diamond':
                    ...
                    nclus.append(np.array(hf['nclu'])) 
                    info_pluss.append(np.array(hf['info_plus']))#22
                ...
                evis.append(np.array(hf['ievent'])) #23
                nclus.append(np.array(hf['nclu'])) 
                info_pluss.append(np.array(hf['info_plus']))#23
                ...
                info_pluss.append(np.array(hf['info_plus']))#24
...
##purge errors
            ...
            Cherry = ph[:,6] ##22
            listChans = [1,2]
            Calo = np.sum(ph[:, listChans],axis = 1) # Ch1 2 → Rino 1 2
            Apc1 = ph[:,3]##22
            Apc2 = ph[:,4]##22
            y1 = xpos[:,0]  ##22 
            x1 = xpos[:,1]  ##22
...
##purge errors
        ...
        return xpos,xinfo,ph,tm,evi,info_plus,ph_cherry1,ph_cherry2,ph_scinti_desy,ph_scinti_after_magnet,ph_calo_photon,ph_calo_elect1,ph_calo_elect2,ph_apc1,ph_apc2,x1,y1,x2,y2,x_cry,y_cry,theta_in_x,theta_in_y